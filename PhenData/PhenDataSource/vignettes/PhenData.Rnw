...
\newline\newline
The IMPC implements a standardized set of phenotyping protocols to generate data. 
...
and parameter for a phenotyping center: \textit{getStrains}, \textit{printStrains}.
