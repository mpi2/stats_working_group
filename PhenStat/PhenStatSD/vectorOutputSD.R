...
vectorOutputSD <- function(phenTestResult, phenotypeThreshold=0.01)
{
	...
	if (method(phenTestResult) %in% c("MM","TF", "SD")) {        
		...
		vectorOutput <- c(paste(framework,", ",fittingMethod, equation,sep=""),
				...
				as.character(analysisResults$model.output.SDtest.pVal), 
				as.character(analysisResults$model.output.Sex.pVal), 
