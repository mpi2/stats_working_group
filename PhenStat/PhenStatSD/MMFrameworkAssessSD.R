	...
	finalResult <- tryCatch({   
				...
				if(keep_batch && keep_equalvar){
					...
					model_SDgenotype <-  do.call("lme", args = list(model_genotype.formula.SDframework,
									random=~1|Batch, x, na.action="na.omit", method="ML"))   # G +G*S 
					
					model_null <- do.call("lme", args=list(model_null.formula.SDframework, x,
									random=~1|Batch, na.action="na.omit",  method="ML"))   #just G
					SDp.value <- (anova(model_SDgenotype, model_null)$"p-value"[2])   # p value for test of SD
					
					model_test_Sex <-  do.call("lme", args = list(test.model_Sex,random=~1|Batch, WTdataOnly, na.action="na.omit", method="ML"))  #just Sex (maybe weight)
					model_null_Sex <- do.call("lme", args=list(null.model_Sex_formula, WTdataOnly,random=~1|Batch, na.action="na.omit",  method="ML"))  # no SEx (maybe weight) 
