		...
		RR_controlPointsThreshold=60, transformValues=FALSE)
{
	...
	if (nchar(stop_message)==0) {
		...
		if (method %in% c("MM","TF", "SD_continuous")){
	...
	if (method=="MM")    { 
		...
		if (outputMessages)
			message(paste("Information:\nMethod: Mixed Model SD assessment.\n",sep="")) 
		
		result <- startModel_SDpipeline(phenListToAnalyse, depVariable, equation, 
				outputMessages, pThreshold, keepList)
		
		## Perform all framework methods 
		if (callAll && is(result,"PhenTestResult")){
			result <- finalModel_SDpipeline(result, outputMessages)  #problem here
		}
