...
##NK tests heterogenous variance ie model structure and whether batch data is available
startModel_SDpipeline <- function(phenList, depVariable, equation="withWeight",
		outputMessages=TRUE, pThreshold=0.05, keepList=NULL)
{
	...
	finalResult <- tryCatch({
				...
				if (batchIn(phenList)){
					...
					## model_test_withoutbatch <- do.call("gls",       args=list(model.formula, x, na.action="na.omit"))
				...
				## functionality by using selected model (no optimisation 
			...
			withWeight = {
				...
				if(numberofsexes==2){
					as.formula(paste(depVariable, "~", paste("Genotype",
											"Sex", "Genotype*Sex","Weight", "Sex*Weight", sep= "+")))
				}else{
					as.formula(paste(depVariable, "~", paste("Genotype",
											"Weight",  sep= "+")))
				}
...
finalModel_SDpipeline <- function(phenTestResult, outputMessages=TRUE)
{
	...
	if(is(phenTestResult,"PhenTestResult")) {
		...
		WTdataOnly <-subset(x, x$Genotype==phenTestResult@refGenotype)   
		...
		if (is.null(linearRegressionOutput$equation) || is.null(depVariable) || is.null(keep_batch)
	...
	## Null model for interaction: genotype*sex is not included
	model_null.formula.SDframework <- switch(linearRegressionOutput$equation,
			withWeight = {
				## Eq.2
				as.formula(paste(depVariable, "~", 
								paste("Sex", "Genotype", "Weight", "Sex*Weight", sep= "+")))
			}
			,
			withoutWeight = {
				## Eq.1
				as.formula(paste(depVariable, "~",
								paste("Sex", "Genotype",  sep= "+")))
				
			}
	)
	## Null model for genotype: genotype + genotype*sex is not included
	model_null.formula.framework <- switch(linearRegressionOutput$equation,
			withWeight = {
				## Eq.2
				as.formula(paste(depVariable, "~", 
			...
			withoutWeight = {
				## Eq.1
				as.formula(paste(depVariable,"Sex", sep="~"))
			}
	)
	
	
	
	## Alternative model for interaction: genotype*sex is included
	model_genotype.formula.SDframework <- switch(linearRegressionOutput$equation,
			withWeight = {
				## Eq.2
				as.formula(paste(depVariable, "~",
								paste("Sex", "Genotype:Sex", "Weight", "Sex*Weight", sep= "+")))               
			},
			withoutWeight = {
	...
	##Alterantive model for sex: sex is included WT data only so no genotype
	test.model_Sex <- switch(linearRegressionOutput$equation,
			withWeight = {
				## Eq.2
				as.formula(paste(depVariable, "~",
								paste("Sex", "Weight", "Sex*Weight", sep= "+")))               
	...
	##null model for sex: sex is excluded WT data only so no genotype
	null.model_Sex_formula <- switch(linearRegressionOutput$equation,
			withWeight = {
				## Eq.2
	...
	finalResult <- tryCatch({   
				...
				if(keep_batch && keep_equalvar){
					...
					model_SDgenotype <-  do.call("lme", args = list(model_genotype.formula.SDframework,
									random=~1|Batch, x, na.action="na.omit", method="ML"))   # G +G*S / G+G*S+ W+ W*S depending on whether weight available
					
					model_null <- do.call("lme", args=list(model_null.formula.SDframework, x,
									random=~1|Batch, na.action="na.omit",  method="ML"))   #just G / G +W+W*S depending on whether weight available
					SDp.value <- (anova(model_SDgenotype, model_null)$"p-value"[2])   # p value for test of SD
					
					model_test_Sex <-  do.call("lme", args = list(test.model_Sex,random=~1|Batch, WTdataOnly, na.action="na.omit", method="ML"))  #just Sex (maybe weight + weight*sex)
					model_null_Sex <- do.call("lme", args=list(null.model_Sex_formula, WTdataOnly,random=~1|Batch, na.action="na.omit",  method="ML"))  # no SEx (maybe weight + weight*sex) 
					Sexp.value <- (anova(model_test_Sex, model_null_Sex)$"p-value"[2])   # p value for test of Sex
					
					model_null2 <- do.call("lme", args=list(model_null.formula.framework, x,
									random=~1|Batch, na.action="na.omit",  method="ML"))  # no G or G*S
					Genotype.p.value <- (anova(model_SDgenotype, model_null2)$"p-value"[2])   # p value for test of genotype
				...
				if(keep_batch && !keep_equalvar){
					...
					SDp.value <- (anova(model_SDgenotype, model_null)$"p-value"[2])
					
					
					model_null2 <- do.call("lme", args=list(model_null.formula.framework, x,
									random=~1|Batch,weights=varIdent(form=~1|Genotype),
									na.action="na.omit",  method="ML"))
					Genotype.p.value <- (anova(model_SDgenotype, model_null2)$"p-value"[2]) 
					
					model_test_Sex <-  do.call("lme", args = list(test.model_Sex,random=~1|Batch,weights=varIdent(form=~1|Genotype), WTdataOnly, na.action="na.omit", method="ML"))
					model_null_Sex <- do.call("lme", args=list(null.model_Sex_formula, WTdataOnly,random=~1|Batch,weights=varIdent(form=~1|Genotype), na.action="na.omit",  method="ML"))
					Sexp.value <- (anova(model_test_Sex, model_null_Sex)$"p-value"[2])   # p value for test of Sex
					
					
					
				}else if(!keep_batch && !keep_equalvar){
					model_SDgenotype <- do.call("gls", args = list(model_genotype.formula.SDframework,
									x,weights=varIdent(form=~1|Genotype),method="ML", na.action="na.omit"))
					
					model_null <- do.call("gls", args=list(model_null.formula.SDframework,
									x,weights=varIdent(form=~1|Genotype),method="ML", na.action="na.omit"))
					
					SDp.value <- (anova(model_SDgenotype, model_null)$"p-value"[2])
					
					model_null2 <- do.call("gls", args=list(model_null.formula.framework, x,
									weights=varIdent(form=~1|Genotype),	na.action="na.omit",  method="ML"))
					Genotype.p.value <- (anova(model_SDgenotype, model_null2)$"p-value"[2]) 
					
					model_test_Sex <-  do.call("gls", args = list(test.model_Sex, weights=varIdent(form=~1|Genotype), WTdataOnly, na.action="na.omit", method="ML"))
					model_null_Sex <- do.call("gls", args=list(null.model_Sex_formula, WTdataOnly, weights=varIdent(form=~1|Genotype), na.action="na.omit",  method="ML"))
					Sexp.value <- (anova(model_test_Sex, model_null_Sex)$"p-value"[2])   # p value for test of Sex
					...
					model_null <- do.call("gls", args=list(model_null.formula.SDframework, x,
					...
					model_test_Sex <-  do.call("gls", args = list(test.model_Sex,random=~1|Batch, WTdataOnly, na.action="na.omit", method="ML"))
					model_null_Sex <- do.call("gls", args=list(null.model_Sex_formula, WTdataOnly,random=~1|Batch, na.action="na.omit",  method="ML"))
					Sexp.value <- (anova(model_test_Sex, model_null_Sex)$"p-value"[2])   # p value for test of Sex
				...
				linearRegressionOutput$model.SDmodel_output <- model_SDgenotype
				linearRegressionOutput$model.SDnull <- model_null
				linearRegressionOutput$model.output.genotype.nulltest.pVal <- Genotype.p.value 
				linearRegressionOutput$model.output.SDtest.pVal <- SDp.value   ######question can I make a new place for this????????? QUESTION
				linearRegressionOutput$model.formula.SDnull <- model_null.formula.SDframework
				linearRegressionOutput$model.formula.SDgenotype <- model_genotype.formula.SDframework   # can I make a nwe place for this or do I use structure exists? 
				linearRegressionOutput$model.output.Sex.pVal <- Sexp.value    ######question can I make a new place for this????????? QUESTION
				
				
				
				## Parse modeloutput and choose output depending on model
				linearRegressionOutput$model.output.summary <- parserOutputSummarySD(linearRegressionOutput)  
				...
				finalResult@analysisResults$model.output.quality <- NA
				finalResult
			},
			...
			error=function(error_mes) {
				...
				if (linearRegressionOutput$equation=="withWeight") 
					stop_message <- paste("Error:\nCan't fit the model ",
							format(model_genotype.formula.SDframework),". Try MM with equation 'withoutWeight'. ",
							"Another option is jitter\n",sep="")
				else
					stop_message <- paste("Error:\nCan't fit the model ",
							format(model_genotype.formula.SDframework),". Try to add jitter or RR plus method.\n",sep="")
...
parserOutputSummarySD<-function(linearRegressionOutput)
{
	...
	lengthoftable <- {
		...
		if (result$equation=="withWeight"){
				...
				table_length <-  6 #intercept, sex, KO*F, KO*M, weight, weight*sex
			}
			else{
				
					
					table_length <- 4  ##intercept, sex, KO*F, KO*M, 
				}
