...
summaryOutput <- function(phenTestResult,phenotypeThreshold=0.01)
{
    ...
    if (phenTestResult$method %in% c("MM","TF")) {
        ...
        if (!is.null(phenTestResult$model.output$male)){
            ...
            results_male[2] <-  paste(format(phenTestResult$model.output$ES_male, nsmall = 0),"%",sep="")
        ...
        if (!is.null(phenTestResult$model.output$female)){
            ...
            results_female[1] <- sprintf("%.4f",round(phenTestResult$model.output$female$p.val, digits=4))
            results_female[2] <-  paste(format(phenTestResult$model.output$ES_female, nsmall = 0),"%",sep="")
