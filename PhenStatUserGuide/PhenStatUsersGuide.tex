...
In the Table \ref{table:01} the example dataset is presented with numerical variables of interest. The Table \ref{table:02} shows the example dataset with categorical data. Mandatory columns are "Genotype" and "Gender". "Assay.Date" column is used to model "Batch" effect if not specified differently. "Weight" column is used to model body weight effect.
...
The PhenStat package uses the following nomenclature for the names of columns: "Gender", "Genotype", "Batch" or "Assay.Date" and "Weight". In addition expected gender values are "Male" and "Female" and missing value is \textit{NA}. 
...
Thus the function \textit{PhenList} requires users to define the reference genotype (mandatory argument \textit{refGenotype} with default value "+\slash+") and test genotype (mandatory argument \textit{testGenotype}). 
...
The user may also specify hemizygotes genotype value (argument \textit{hemiGenotype}) then hemizygotes are treated as test genotype. 
...
With the dataset described in Table \ref{table:03} when \textit{hemiGenotype} argument of the PhenList function is defined as "KO\slash Y" the actions of the function are:  "KO/Y" genotypes are relabelled to "KO/KO" for males;  females "+\slash KO" heterozygous are filtered out. 
...
\begin{figure}[!htpb]%figure01
\centerline{\includegraphics[scale=0.5]{Motivation1.jpg}}
...
\begin{figure}[!htpb]%figure01
\centerline{\includegraphics[scale=0.5]{Motivation2.png}}
