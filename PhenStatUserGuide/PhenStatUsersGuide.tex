...
\begin{itemize}
\item If the 95\% confidence interval for $\lambda$ includes 1 then the implementation returns "lambda=NA" for the transformation value as no transformation is needed.
\item If the $\lambda$ value is between -0.1 or 0.1 then the value is classed as 0 and the implementation returns "lambda=0" which indicate that a log transformation is required. 
